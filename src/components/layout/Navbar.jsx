...
export default function Navbar() {
  ...
  return (
    <>
      {/* Top info bar */}
      <div className={`hidden md:block transition-all duration-300 ${transparent ? 'bg-navy-900/60 backdrop-blur-sm' : 'bg-navy-900'}`}>
        <div className="max-w-7xl mx-auto px-6 py-2 flex items-center justify-between">
          <div className="flex items-center gap-6 text-xs text-white/60" style={{ fontFamily: 'Barlow, sans-serif' }}>
            <span className="flex items-center gap-1.5">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg>
              4/5 Apapa–Oshodi Expressway, Apapa, Lagos
            </span>
            <span className="flex items-center gap-1.5">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 11.5 19.79 19.79 0 01.1 2.82 2 2 0 012.11 1h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L6.91 8.09A16 16 0 0014.36 15.54l.55-.45a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 17.38z"/></svg>
              0803 718 5967
            </span>
          </div>
          <div className="flex items-center gap-4 text-xs text-white/60" style={{ fontFamily: 'Barlow, sans-serif' }}>
            <a href="mailto:info@raisevisionlogistics.com" className="hover:text-white transition-colors">
      ...
      {/* Main nav */}
      <header className={`sticky top-0 z-50 transition-all duration-400 ${bg} ${border}`}
        ...
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between py-3">
          ...
          <button onClick={() => setOpen(!open)} className="lg:hidden p-2 flex flex-col gap-1.5">
            {[0,1,2].map(i => (
              <span key={i} className={`block w-6 h-0.5 transition-all duration-300 ${transparent ? 'bg-white' : 'bg-navy-900'} ${
        ...
        {/* Mobile menu */}
        <div className={`lg:hidden bg-white border-t border-gray-100 overflow-hidden transition-all duration-400 ${open ? 'max-h-[500px]' : 'max-h-0'}`}>
